...
resource "google_compute_security_policy" "policy" {
  ...
  dynamic "rule" {
    ...
    content {
      ...
      match {
        expr {
          expression = lookup(rule.value, "exclude_ip", null) == null ? "evaluateThreatIntelligence('${rule.value["feed"]}')" : "evaluateThreatIntelligence('${rule.value["feed"]}', [%{for index, i in rule.value["exclude_ip"]}'${i}'%{if index < length(rule.value["exclude_ip"]) - 1},%{endif}%{endfor}])" # using string directive
        }
      }
