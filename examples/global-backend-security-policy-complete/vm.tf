...
resource "google_service_account" "vm_service_account" {
  project      = var.project_id
  account_id   = "global-ca-web-svc-act"
...
resource "google_compute_instance" "ca_vm_1" {
  name                      = "global-ca-test-vm-${data.google_compute_zones.available_primary.names[0]}"
...
resource "google_compute_instance_group" "ca_vm_1_ig" {
  name        = "global-ca-ig-${data.google_compute_zones.available_primary.names[0]}"
